...
def sum_times(t1, t2):
    '''Sum two time objects using time_to_sec and sec_to_time.'''
    total_seconds = time_to_sec(t1) + time_to_sec(t2)
    return sec_to_time(total_seconds)

def change_time(time, seconds):
    '''Modify a time object by adding or subtracting seconds using time_to_sec and sec_to_time.'''
    total_seconds = time_to_sec(time) + seconds
    if total_seconds < 0:
        total_seconds = 0  # Ensure time doesn't become negative
